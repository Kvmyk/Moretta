...
Configure API keys, PostgreSQL connection settings, and the vault encryption key in the `.env` file.
...
DATABASE_BACKEND | postgres | Storage backend (`postgres` or `sqlite`)
DATABASE_URL | postgresql://moretta:moretta@postgres:5432/moretta | Main database DSN used by backend storage and vault
...
Legacy migration: if you already have `data/store.db` and `data/vault.db`, use `python backend/scripts/migrate_sqlite_to_postgres.py --store-db data/store.db --vault-db data/vault.db --database-url postgresql://moretta:moretta@localhost:5432/moretta --encryption-key <your-key>`.
...
- Storage: PostgreSQL (default) with SQLite fallback for tests/local debug
